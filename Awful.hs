...
check ::
  [String] ->
  (Files, Locations, Defs, Map' Kind) ->
  Location' ->
  String ->
  IO (Err (Files, Locations, Defs, Map' Kind, File))
...
check_imports ::
  [String] ->
  (Files, Locations, Defs, Map' Kind, File) ->
  [(Location', String)] ->
  IO (Err (Files, Locations, Defs, Map' Kind, File))
...
eval'' a b = do
  ...
  return (c >>= \(_, e, f, j, (File _ g h i)) -> tokenise_parse_naming_typing_eval e j (g, h, i) f b)
main :: IO ()
main = do
  args <- getArgs
  case args of
