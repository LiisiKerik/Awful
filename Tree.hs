...
module Tree where
  ...
  parse_op_1 = filter_parser (\x -> notElem x ["->"]) parse_op''
  ...
  parse_op_type =
    (
      Op_type_0 <$>
      parse_br_type' <*>
      parse_some ((,) <$> (Name <&> filter_parser (\x -> elem x ["*", "+", "->"]) parse_op'') <*> parse_br_type'))
  ...
  parse_pat_and_type =
    (
      (\a -> first ((,) a)) <$>
      parse_br_pat' <*>
      ((,) [] <$ parse_colon <*> parse_type <+> (\a -> first (\(b, c) -> (a, b) : c)) <$> parse_op <*> parse_pat_and_type))
